...
UMaterial* UWindMaterialBuilder::CreateWindMaterial(const FString& SavePath)
{
	...
	WindWPO->Code = TEXT(
		...
		"        float WindNorm = saturate(Speed / 600.0f);\n"
		"        float LeanStrength = smoothstep(0.1f, 1.0f, WindNorm);\n"
		"        float BendAmount = min(Speed, 600.0f) * 0.06 * FlexGrad2;\n"
		"        Result += Dir * BendAmount;\n"
		"\n"
		"        // Motor shiver (attenuated in strong wind to avoid twitching)\n"
		"        float WindShiver = sin(Time * 8.0 + Hash * 80.0) * Speed * 0.004 * FlexGrad * lerp(1.0f, 0.25f, LeanStrength);\n"
		"        Result += Dir * WindShiver;\n"
		"\n"
		"        // Extra downward lean under strong wind force\n"
		"        Result.z += -BendAmount * 0.35f * LeanStrength;\n"
		"    }\n"
		"}\n"
		"\n"
		"return Result;"
