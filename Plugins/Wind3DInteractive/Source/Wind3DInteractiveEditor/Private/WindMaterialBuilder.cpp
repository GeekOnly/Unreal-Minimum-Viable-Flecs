...
UMaterial* UWindMaterialBuilder::CreateWindMaterial(const FString& SavePath)
{
	...
	WindWPO->Code = TEXT(
		...
		"// Large bucket (80cm) so all verts of one blade share the same hash\n"
		"float2 BladeXY = floor(WorldPos.xy / 80.0);\n"
		"float Hash = frac(sin(dot(BladeXY, float2(127.1, 311.7))) * 43758.5453);\n"
		"float Hash2 = frac(sin(dot(BladeXY, float2(269.5, 183.3))) * 76123.8731);\n"
		"\n"
		"// --- Layer 1: Primary sway (slow traveling wave) ---\n"
		"float WavePhase1 = Time * 1.1 + WorldPos.x * 0.004 + WorldPos.y * 0.003;\n"
		"float Sway1 = sin(WavePhase1 + Hash * 6.28) * 10.0;\n"
		"float Sway1Y = sin(WavePhase1 * 0.7 + Hash2 * 6.28 + 1.57) * 7.0;\n"
		"\n"
		"// --- Layer 2: Secondary ripple (faster, smaller) ---\n"
		"float WavePhase2 = Time * 2.4 + WorldPos.x * 0.01 + WorldPos.y * 0.007;\n"
		"float Sway2 = sin(WavePhase2 + Hash * 3.14) * 4.0;\n"
		"float Sway2Y = cos(WavePhase2 * 1.1 + Hash2 * 3.14) * 3.0;\n"
		"\n"
		"// --- Layer 3: Micro-shiver (high freq, very small) ---\n"
		"float ShiverPhase = Time * 6.0 + Hash * 80.0;\n"
		"float Shiver = sin(ShiverPhase) * 1.5;\n"
		"float ShiverY = cos(ShiverPhase * 1.3 + 0.7) * 1.0;\n"
		"\n"
		"// --- Layer 4: Gust wave (periodic sweep across field) ---\n"
		"float GustWave = sin(Time * 0.35 + WorldPos.x * 0.002 - WorldPos.y * 0.001);\n"
		"float GustStrength = saturate(GustWave * 2.0 - 0.5);\n"
		"float GustPush = GustStrength * 15.0;\n"
		"\n"
		...
		"float IdleZ = sin(Time * 1.5 + Hash * 6.28) * 1.5 * FlexGrad2;\n"
		...
		"\n"
		"        // Gentle bend — grass is ~30-50cm tall, max ~30cm displacement\n"
		"        float BendAmount = min(Speed, 600.0f) * 0.06 * FlexGrad2;\n"
		"        Result += Dir * BendAmount;\n"
		"\n"
		"        // Wind-driven shiver\n"
		"        float WindShiver = sin(Time * 8.0 + Hash * 80.0) * Speed * 0.005 * FlexGrad;\n"
